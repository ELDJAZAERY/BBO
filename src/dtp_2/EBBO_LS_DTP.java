...
public class EBBO_LS_DTP {
	...
	public void BBO_S(LinkedList<Vertex> vertices, Graph graph) {
		...
		for (int i = 0; i < popsize; i++) {
			...
			Individual I = new Individual(individual, i, 0);
			population.add(I);
		...
		// Compute corresponding HSI, µ, λ values
		...
		// Prob(i) is the probability of having a rank k(i)
		Prob();
		...
		for (int i = 0; i < Maxgen; i++) {
				...
				if (r < lamda.get(j)) {
					for (int l = 0; l < popsize && l != j; l++) {
						...
						if (r < mu.get(l)) {
							...
							/*-------------------------------------------------------------------------------------------------*/
							LinkedList<Obj_Dec> S1 = new LinkedList<Obj_Dec>();
							for(int s = 0; s < NbNodes; s++)
							{
								Obj_Dec obj = new Obj_Dec();
								obj.exist = true;
								obj.var = population.get(r1).sol.permutation.get(s);
								S1.add(obj);
							}
							...
							for(int s = 0; s < NbNodes; s++)
							{
								if (population.get(r1).sol.permutation.get(s) == population.get(r2).sol.permutation.get(s))
								{	
									S1.get(s).exist = false;
							...
							/*-------------------------------------------------------------------------------------------------*/
							LinkedList<Obj_Dec> A1 = new LinkedList<Obj_Dec>();
							for(int s = 0; s < NbNodes; s++)
							{
								Obj_Dec obj = new Obj_Dec();
								obj.exist = true;
								obj.var = population.get(l).sol.permutation.get(s);
								A1.add(obj);
							}
							...
							for(int s = 0; s < NbNodes; s++)
							{
								if (population.get(l).sol.permutation.get(s) != S.get(s))
								{	
									
									A1.get(s).exist = false;
							...
							int d = rand.nextInt(population.get(j).sol.verticesDT.size() - 1) + 0;
			...
			// Compute immigration rate and emigration rate for each species
			// count
			GetLambdaMu_set();
	...
	public void GetSpeciesCounts() {
		for (int i = 0; i < population.size(); i++) {
			population.get(i).SpeciesCount = P - i;
		}
	}

	public void GetLambdaMu() {
		for (int i = 0; i < population.size(); i++) {
	...
	public void Prob_2() {
		...
		for (int k = 0; k < popsize; k++) {
			if (k == 0) {
				...
				prob.add(sommeL / (sommeM * (1 + sommeT)));
			}
		}
	}

	public void Prob_2_set() {
		float sommeLT = 0;
		float sommeMT = 0;
		float sommeT = 0;
		...
		for (int k = 0; k < popsize; k++) {
			if (k == 0) {
				...
				prob.set(1, sommeL / (sommeM * (1 + sommeT)));
			}
		}
	}
